...
export async function POST(request: NextRequest) {
    try {
        ...
        const sheet = workbook.addWorksheet('Age Segments');

        sheet.columns = [
            { header: 'Age Range', key: 'label', width: 25 },
        ...
        return new NextResponse(buffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="age-segmentation.xlsx"`
            }
        });
    } catch (e) {
        return NextResponse.json({ error: 'Failed to export age segmentation', details: String(e) }, { status: 500 });
    }
}
